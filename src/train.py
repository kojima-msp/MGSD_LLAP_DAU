...
for N_layers in N_layers_list:
    ...
    os.makedirs(f'./data/{datatype}/trained_params/nlayers{N_layers:02d}/{model_name}', exist_ok=True)
        ...
        for epoch in range(N_epochs):
            for train_matfile_idx, train_matfile in enumerate(train_matfile_list):
                ...
                for noise_idx, noise in enumerate(noise_list):
                    ...
                    print(model_name, f'nlayers{N_layers:02d}', f'nepochs{epoch:02d}', train_matfile[-11:-4], f'{noise:.03f}', np.sqrt(loss.cpu().detach().numpy()))
                
        torch.save(model.state_dict(), f'./data/{datatype}/trained_params/nlayers{N_layers:02d}/{model_name}/fortest{cv_idx}.pth')